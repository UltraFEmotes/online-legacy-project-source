					   ...
					   _In_ int       nCmdShow)
{
	...
	printf("==========================================\n");
	printf("   Minecraft Online Multiplayer Client    \n");
	printf("==========================================\n");
	...
	while (true)
	{
		...
		{
			input[strcspn(input, "\r\n")] = 0;
			if (strlen(input) > 0 && strlen(input) <= 16)
			{
				strncpy_s(g_Win64Username, 17, input, _TRUNCATE);
				break;
			}
			printf("Username must be 1-16 characters. Try again:\n> ");
		}
	}
	printf("Welcome, %s!\n", g_Win64Username);

	// Step 2: Relay Server Address
	printf("\nEnter the Relay Server address (IP PORT or URL), e.g. 123.45.67.89 25565 or https://example.com/");
	printf("\nPress Enter to use 127.0.0.1 25565 (local testing).\n> ");
	{
		char hostInput[256];
		if (fgets(hostInput, sizeof(hostInput), stdin))
		{
			hostInput[strcspn(hostInput, "\r\n")] = 0;
			if (strlen(hostInput) > 0)
			{
				// Parse URL format: http://hostname:port/ or https://hostname:port/
				std::string inputStr(hostInput);
				std::string hostname;
				int port = 25565;

				// Strip http:// or https:// prefix
				if (inputStr.rfind("https://", 0) == 0) {
					inputStr = inputStr.substr(8);
					port = 443; // Default HTTPS port
				} else if (inputStr.rfind("http://", 0) == 0) {
					inputStr = inputStr.substr(7);
				...
				// Check if port is specified in URL (hostname:port)
				size_t colonPos = inputStr.rfind(':');
				if (colonPos != std::string::npos) {
					// Make sure it's not just a port number entered directly
					std::string potentialPort = inputStr.substr(colonPos + 1);
					if (potentialPort.find_first_not_of("0123456789") == std::string::npos) {
						hostname = inputStr.substr(0, colonPos);
						port = atoi(potentialPort.c_str());
					} else {
						hostname = inputStr;
					}
				} else {
				...
				// Check if user entered "IP PORT" format (space-separated)
				char* spacePos = strchr(hostInput, ' ');
				if (spacePos != nullptr && inputStr.find("://") == std::string::npos) {
					// User entered IP PORT format, use original parsing
					char* customIp = hostInput;
					char* customPortStr = spacePos + 1;
					int customPort = atoi(customPortStr);
					WinsockNetLayer::SetCustomHostAddress(customIp, customPort);
					printf("Relay Server set to %s:%d\n", customIp, customPort);
				} else {
					// URL format parsed above
					WinsockNetLayer::SetCustomHostAddress(hostname.c_str(), port);
					printf("Relay Server set to %s:%d\n", hostname.c_str(), port);
				}
			}
			else
			{
				printf("Using local Relay Server at 127.0.0.1:25565\n");
			}
		}
	}
	
	// Free console after successful login
	FreeConsole();
