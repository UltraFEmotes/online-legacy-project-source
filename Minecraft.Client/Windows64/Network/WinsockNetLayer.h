...
#define WIN64_NET_RECV_BUFFER_SIZE 8388608
...
public:
	...
	static void SetSupabaseCredentials(const std::string& token, const std::string& userId);
	...
	static HttpResponse DoWinHttpRequest(const wchar_t* host, const std::wstring& path, const wchar_t* method, const std::string& requestData, const std::vector<std::wstring>& headers);
