...
bool Minecraft::handleClientSideCommand(const wstring& chatMessage)
{
	...
	wstring cmd;
	wstring args;
	size_t spacePos = chatMessage.find(L' ');
	if (spacePos != wstring::npos)
	{
		cmd = chatMessage.substr(0, spacePos);
		args = chatMessage.substr(spacePos + 1);
		// Trim leading spaces from args
		size_t start = args.find_first_not_of(L' ');
		if (start != wstring::npos) args = args.substr(start);
		else args = L"";
	}
	else
	{
	...
	wstring cmdLower = cmd;
	for (size_t i = 0; i < cmdLower.size(); i++)
		cmdLower[i] = towlower(cmdLower[i]);

	int iPad = 0;
	if (player) iPad = player->GetXboxPad();

	// ---- /gamemode (mode) ----
	if (cmdLower == L"/gamemode")
	{
		if (args.empty())
		{
			if (gui) gui->addMessage(L"\xa7c[Usage] /gamemode <survival|creative|adventure|0|1|2>", iPad);
			return true;
		}
		...
		GameType *newMode = NULL;
		wstring argsLower = args;
		for (size_t i = 0; i < argsLower.size(); i++)
			argsLower[i] = towlower(argsLower[i]);

		if (argsLower == L"survival" || argsLower == L"s" || argsLower == L"0")
			newMode = GameType::SURVIVAL;
		else if (argsLower == L"creative" || argsLower == L"c" || argsLower == L"1")
			newMode = GameType::CREATIVE;
		else if (argsLower == L"adventure" || argsLower == L"a" || argsLower == L"2")
			newMode = GameType::ADVENTURE;

		if (newMode == NULL)
		{
			if (gui) gui->addMessage(L"\xa7c[Error] Unknown gamemode: " + args, iPad);
			return true;
		}

		// Apply to the local host player
		if (player)
		{
			player->chat(L"\xa7e[Server] Set own game mode to " + newMode->getName());
		}

		// Set the world default gamemode via the app's host options
		app.SetGameHostOption(eGameHostOption_GameType, newMode->getId());
	...
	if (cmdLower == L"/op")
	{
		if (args.empty())
		{
			if (gui) gui->addMessage(L"\xa7c[Usage] /op <player name>", iPad);
			return true;
		}
		...
		// Broadcast the op status
		if (player)
		{
			player->chat(L"\xa7e[Server] Opped player: " + args);
		}
	...
	// Unknown command
	if (gui) gui->addMessage(L"\xa7c[Error] Unknown command: " + cmd, iPad);
	return true;
}
