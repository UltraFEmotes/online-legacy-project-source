...
static DWORD s_lastSendTime[256] = {0};
static DWORD s_fakeQueueSize[256] = {0};
...
void IQNetPlayer::SendData(IQNetPlayer * player, const void* pvData, DWORD dwDataSize, DWORD dwFlags)
{
	if (WinsockNetLayer::IsActive())
	{
		...
		s_fakeQueueSize[player->m_smallId] += dwDataSize;
	}
}
...
DWORD IQNetPlayer::GetSendQueueSize(IQNetPlayer * player, DWORD dwFlags) { 
	if (!player) return 0;
	BYTE id = player->m_smallId;
	DWORD now = GetTickCount();
	DWORD elapsed = now - s_lastSendTime[id];
	s_lastSendTime[id] = now;
	
	// Drain at 1MB/sec (1024 bytes per ms)
	DWORD drain = elapsed * 1024;
	if (drain > s_fakeQueueSize[id]) s_fakeQueueSize[id] = 0;
	else s_fakeQueueSize[id] -= drain;

	return s_fakeQueueSize[id];
}
...
void Win64_SetupRemoteQNetPlayer(IQNetPlayer * player, BYTE smallId, bool isHost, bool isLocal)
{
	...
	player->SetCustomDataValue(1); // Mark as active so game engine sends chunks
